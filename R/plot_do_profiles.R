#' plot_do_profiles
#'@description
#' creates a plot showing dissolved oxygen profiles (figure S3)
#'
#'
#' @return
#' stores a pdf file "doprofiles.pdf" in graphics folder
#'
#'
#'
#' @export
...
par(mfrow = c(2,5), omi = c(0.1, 0.5, 0.7, 0.1), mar = c(5, 1, 1, 5))
  ...
  for (yr in 1:2) {
    ...
    axis(side = 3, at = seq(0, 12, by = 4), cex.axis = 1.25)
    if (mo ==1) mtext(side = 2, "Depth (m)", line = 3)
    if (yr == 1) mtext( side = 3, expression(paste("Dissolved Oxygen (mg l"^"-1",")")), line = 2)
    ...
    for (site in 1:4) {
      ...
      if (length(unlist(plot.indices)) == 1)
        plot.o2.data <- DO[, unlist(plot.indices)]
      if (length(unlist(plot.indices)) > 1)
        plot.o2.data <- rowMeans(DO[, unlist(plot.indices)], na.rm = T)



      nrows <- length(plot.o2.data)
      lines(
        x = plot.o2.data,
        y = -1:-nrows,
        lwd = 2,
